...
<div class="navbar">
	...
	<div class="container-inner">
		...
		<form class="form-horizontal" name="tweet" action="/zebrinho/search" method="post">
			<center>
			<input type="text" placeholder="User Name" value="${ searchUsername }" style="height:26px;"class="input-xlarge" name="username" /> 
		...
		<ul>
			  <c:forEach var="user" items="${users}">
        		<li style="margin-bottom:20px;"> <div><a href="/zebrinho/user/${ user.username }">${ user.username }</a>
        		<div>Following <span class="label label-success">${ user.following }</span> person and followed by  <span class="label label-info">${ user.followers }</span> person</div>
        		</div></li>
      		  </c:forEach>
		</ul>
	</div></div>
